...
def image_links(request):
    return render(request, 'core/image_links.html')
...
def add_product(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            product = form.save()
            return redirect('core/add_product')  # Redirect to a list of products or a success page
    else:
        form = ProductForm()
...
def view_schema(request):
    ...
    return render(request, 'core/view_schema.html', {'schema_data': schema_data})
